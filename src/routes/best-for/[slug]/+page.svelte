<script lang="ts">
	...
	let {
		...
		data: {
			...
			highlights: { total: number; affiliateReady: number; freeTrial: number; topCategory: string };
		};
	} = $props();
...
<main class="max-w-[1280px] mx-auto px-6 py-8 lg:py-12" in:fly={{ y: 12, duration: 280 }}>
		...
		<div class="space-y-5">
			...
			<header class="space-y-4">
				...
				<p class="max-w-2xl text-base lg:text-lg text-metal-500 leading-relaxed">
					{data.description}
					For freelancers, the list favors tools that help you pitch better, write faster, and ship with
					less friction.
				</p>
			</header>
		</div>
		...
		<div class="grid gap-4 self-start">
			<div class="border border-metal-100 bg-surface rounded-sm p-5">
				...
				<div class="mt-4 grid grid-cols-2 gap-3">
					<div
						class="rounded-sm border border-metal-100 bg-[color-mix(in_srgb,var(--surface)_94%,var(--metal-100)_6%)] p-3"
					>
					...
					>
						...
						<p class="mt-2 text-lg font-heading text-metal-900 capitalize">
	...
	<section class="mt-8 grid gap-8 xl:grid-cols-[1.12fr_0.88fr]">
		<div class="space-y-4">
			<div class="flex items-center justify-between">
				<h2 class="text-xs uppercase tracking-[0.3em] text-metal-500 font-mono">
					Top recommendation
				</h2>
				<span class="text-xs uppercase tracking-[0.2em] text-metal-500 font-mono"
					>primary CTA first</span
				>
			</div>
			...
			{#if data.recommendations[0]}
				...
				<div
				class="block border border-metal-900 bg-metal-100 text-metal-900 rounded-sm p-6 lg:p-7 shadow-[6px_6px_0px_var(--accent)] transition-all hover:-translate-y-[2px]"
				>
					<div class="flex items-start justify-between gap-4">
						...
						<div class="shrink-0 text-right">
							...
							<p class="text-4xl font-mono font-bold text-accent">
								{tool.score ?? tool.trustScore ?? 0}
							</p>
						</div>
					</div>
					<div
						class="mt-6 flex flex-wrap items-center gap-3 text-xs uppercase tracking-[0.2em] font-mono"
					>
						<a href={toolHref(tool)} class="badge-hero">View tool</a>
						<a href={compareHref(tool, data.recommendations[1] ?? tool)} class="badge-hero"
							>Compare</a
						>
						{#if tool.affiliateAvailable}
		...
		<aside class="space-y-4">
			<div
				class="border border-metal-100 bg-[color-mix(in_srgb,var(--surface)_94%,var(--metal-100)_6%)] rounded-sm p-4"
			>
				...
				<div
					class="mt-4 flex items-center justify-between rounded-sm border border-metal-100 bg-[color-mix(in_srgb,var(--surface)_94%,var(--metal-100)_6%)] px-3 py-2"
				>
		...
		{#each data.recommendations as tool, index}
			...
			>
				<div class="flex items-start justify-between gap-4">
					...
					<p class="text-2xl font-mono font-bold text-metal-900">
						{tool.score ?? tool.trustScore ?? 0}
					</p>
				</div>
				...
				<div
					class="mt-4 flex items-center justify-between text-xs uppercase tracking-[0.2em] font-mono text-metal-500"
				>
					<span>View</span>
				...
				{#if data.recommendations[index + 1]}
					<div class="mt-4 border-t border-metal-100 pt-4">
						<div
							class="inline-flex items-center gap-2 text-xs uppercase tracking-[0.2em] font-mono text-metal-900"
						>
							<span>Compare</span>
